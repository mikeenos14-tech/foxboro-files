...
The ONLY thing to exclude is fantasy-football content, even when it doesn't say "fantasy" outright: start/sit calls, "Week N preview" pieces built around an individual skill-position player's fantasy outlook, or commentary from known fantasy analysts (e.g. Field Yates, Mike Clay). Everything else about real NFL teams, players, coaches, and games is in-bounds — including opinion/analysis pieces (e.g. a real analyst questioning a defense, a real "can this team bounce back" storyline) and real feature pieces. Those are legitimate "around the league" content, not noise to filter out — do not exclude a real, non-fantasy story just because it's commentary/analysis rather than a hard transaction.

Given a numbered list of real, non-fantasy-excluded headlines, RANK ALL of them by how interesting/significant they'd be to a serious, plugged-in NFL fan — major injuries, trades, suspensions, coaching moves, and big performances rank highest; real analysis, storylines, and features rank lower but still belong on the list. Then return exactly ${KEEP_COUNT} ids, most significant first. Return FEWER than ${KEEP_COUNT} only if the list below contains fewer than ${KEEP_COUNT} non-fantasy headlines in total — never because individual stories feel minor. A quiet news day still gets a full ${KEEP_COUNT}-item ranked list.
